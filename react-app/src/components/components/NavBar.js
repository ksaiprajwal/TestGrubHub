...
const NavBar = ({ setSearch }) => {
  ...
  const handleSearchSubmit = async (e) => {
    ...
    let businessResults;
    const fuse = new Fuse(Object.values(businesses), options);
    const searchValue = document.getElementById("search-input-field-business-list").value;

    if (searchValue === "") {
      businessResults = Object.values(businesses);
    } else if (["San Francisco", "New York", "Brooklyn"].includes(searchValue)) {
      businessResults = Object.values(businesses).filter(business => business.city === searchValue);
    } else {
      const results = fuse.search(searchValue).slice(0, 15);
      businessResults = results.map(result => result.item);
    }
    setSearch(businessResults);
  ...
  if (sessionUser) {
    sessionLinks = (
      <>
        <div id="search-restaurant-button">
          <NavLink to='/search-restaurants' exact={true} activeClassName='active' id='search-nav'>
            Search By Location
          </NavLink>
        </div>
        <div id="admin-login-button">
          <NavLink to='/admin-login' exact={true} activeClassName='active' id='admin-login-nav'>
            Admin Login
          </NavLink>
        </div>
        <div id="for-businesses-button-logged">
            ...
            <div id="dropdown-parent-container">
              <div id="dropdown-upper-div">
                ...
                <div id="dropdown-links-container">
                  <Link id="about-link" to={`/user-profile/reviews`}>
                    <div className="dropdown-links" id="dropdown-links-business-navbar">
                      <img className='icon-img-asset' id="icon-img-business-navbar" alt='about me' src={userprofileicon} />
                      <div>About Me</div>
                    </div>
                  </Link>
                  <div onClick={logout} className="dropdown-links" id="dropdown-links-business-navbar">
